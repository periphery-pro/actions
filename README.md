...
| `xcode_version` | string | empty | Xcode version to build with, for example `26.3`. It must match an `/Applications/Xcode_<version>.app` install on the [macos-15 runner image](https://github.com/actions/runner-images/blob/main/images/macos/macos-15-Readme.md); the job fails and lists the installed versions otherwise. By default the runner's preselected Xcode is used. |

...
| `runner` | string | `macos-latest` | GitHub-hosted macOS runner label to scan on, for example a pinned image such as `macos-15`, or a [larger runner](https://docs.github.com/en/actions/using-github-hosted-runners/using-larger-runners) label such as `macos-15-xlarge` where the repository has access to one. The `macos-latest` default follows GitHub's alias as it migrates to newer images; pin an explicit image to keep results stable across that migration. |
